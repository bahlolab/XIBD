...
The input for XIBD is unphased haplotype data for SNPs in PLINK PED and MAP format (http://pngu.mgh.harvard.edu/~purcell/plink/data).
...
If `model=2` then a PLINK LD file is also required, see http://pngu.mgh.harvard.edu/~purcell/plink/ld for more details about this 
...
HapMap phase 2 and 3 PED, MAP and LD reference data (hg19/build 37) for the 11 HapMap populations can be downloaded from link/to/web.
**NOTE: XIBD should not be used with small reference datasets; reference populations that do not match the input population or cohorts of mixed populations.**
...
Below is an IBD analysis performed on simulated data to demonstrate the steps involved in XIBD. 
...
my_genotypes <- getGenotypes(ped.map = example_pedmap,
                             reference.ped.map = reference_pedmap,
                             snp.ld = NULL,
                             model = 1,
                             maf = 0.01,
...
To correct for this, \code{switchBOTgenotypes()} switches the A and B alleles in the filtered dataset for all SNPs corresponding to 
Illumina BOT designations. 
This mean a homozygous genotype, 0, will be changed to a homozygous alternative genotype, 2, and vis versa. 
Heterozygous genotypes will be unchanged.

For the purpose of this example, we simulated data using Illumina's naming convention and the reference data is HapMap reference data.
...
my_genotypes_2 <- switchBOTgenotypes(ped.genotypes = my_genotypes, 
                                     topbot.annotation = reference_topbot)
...
# get IBD proportions
locus_matrix <- getLocusMatrix(ped.genotypes = my_genotypes_2, 
                               ibd.segments = my_ibd)
locus_prop <- getLocusProportion(ped.genotypes = my_genotypes_2, 
                                 locus.matrix = locus_matrix, 
                                 groups = NULL)

# plot IBD proportions
plotIBDproportions(locus.proportions = locus_prop, 
                   interval = NULL,
                   annotation.genes = NULL, 
                   highlight.genes = NULL, 
                   add.rug = TRUE,
